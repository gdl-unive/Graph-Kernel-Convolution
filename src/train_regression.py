...
import wandb
from dataset import EgonetLoader, TUDataset, ZINCDataset
from model import *
...
def run_training_process_with_validation(run_params):
    ...
    if run_params.dataset == 'ZINC':
        ...
        run_params.loss = torch.nn.L1Loss()
        run_params.num_classes = int(1)
        ...
        run_params.num_classes = int(train_dataset.num_classes)

    train_loader = EgonetLoader(train_dataset[:], batch_size=run_params.batch_size, shuffle=True)
    val_loader = EgonetLoader(val_dataset, batch_size=5000, shuffle=False)
    test_loader = EgonetLoader(test_dataset, batch_size=5000, shuffle=False)
...
def get_arg_parser():
    ...
    parser.add_argument("--project", default='FullBack_ZINC')
    ...
    parser.add_argument("--slurm", default=False, type=lambda x: (str(x).lower() == 'true'))  # num filters
    return parser
